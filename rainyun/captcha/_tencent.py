"""腾讯滑块验证码 — 本地 CV 方案 + 远程 YOLO API。

TencentCaptchaProvider 优先调用远程 YOLO 定类 API（_yolo.py solve_points 封装，
模型部署在 huasuanyun 服务器），失败或未配置时降级到 ddddocr + OpenCV 本地链路。
核心流程：下载图片 → YOLO API 定类点击坐标（或 OCR 检测候选框 + 多模态评分匹配）→ 点击提交。
"""
...
class TencentCaptchaProvider:
    """腾讯滑块验证码 — 远程 YOLO API + 本地 OpenCV 降级方案。"""
    ...
    def solve(self, driver, timeout, retry_stats, logger_adapter):
        ...
        2. 下载大图和 sprite
        3. 优先调用远程 YOLO API 定类（CAPTCHA_API_URL 配置时）
        4. 失败/未配置 → ddddocr det 提取候选框 + 多模态评分匹配（形状 + OCR 语义 + SIFT）
        5. 精修定位 → 点击 → 确认
        ...
        try:
            ...
            yolo_points = self._try_yolo_api(logger_adapter)
            if yolo_points:
                final_click_positions = yolo_points
                best_total_score = 3.0
                logger_adapter.info(f"✅ YOLO API 定类成功，点击坐标: {final_click_positions}")
            else:
                # ===== 旧链路：ddddocr det + 多模态评分 + Siamese 降级 =====
    ...
    def _try_yolo_api(self, logger_adapter):
        """调用远程 YOLO 定类 API，成功返回 3 个点击坐标，失败返回 None。

        通过环境变量配置：
        - CAPTCHA_API_URL      API 地址（设置后启用 YOLO 主链路；留空走本地降级）
        - CAPTCHA_API_TOKEN    API 鉴权 token（可选）
        - CAPTCHA_USE_YOLO_API 设为 0 强制禁用 YOLO 链路（回滚开关）
        """
        if os.getenv("CAPTCHA_USE_YOLO_API", "1").strip() == "0":
            return None
        ...
        try:
            with open("temp/captcha.jpg", "rb") as f:
                captcha_b = f.read()
            with open("temp/sprite.jpg", "rb") as f:
                sprite_b = f.read()
        except OSError as e:
            logger_adapter.warning(f"读取验证码图片失败，跳过 YOLO API: {e}")
            return None
        ...
        try:
            resp = requests.post(
                ...
                headers=headers,
                files={
                    "captcha": ("captcha.jpg", captcha_b, "image/jpeg"),
                    "sprite": ("sprite.jpg", sprite_b, "image/jpeg"),
                },
                timeout=30,
            )
            if resp.status_code != 200:
                logger_adapter.warning(
                    f"YOLO API 返回 {resp.status_code}: {resp.text[:200]}"
                )
                return None
            data = resp.json()
            points = data.get("points", [])
            if data.get("ok") and len(points) == 3:
                return points
            logger_adapter.warning(f"YOLO API 定类失败: {data.get('error', 'unknown')}")
            return None
        except Exception as e:
            logger_adapter.warning(f"YOLO API 调用异常: {e}")
            return None
